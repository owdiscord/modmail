...
export class Thread {
  ...
  async getThreadMessages(): Promise<ThreadMessage[]> {
    ...
    return threadMessages.map((row: any) => new ThreadMessage(row));
  }
  ...
  async scheduleClose(time: string, user: User, silent: number): Promise<void> {
    const closed_username = useDisplaynames
      ? user.globalName || user.username
      : user.username;

    await this
      .db`UPDATE threads SET scheduled_close_at = ${time}, scheduled_close_id = ${user.id}, scheduled_close_name = ${closed_username}, scheduled_close_silent = ${silent} WHERE id = ${this.id}`;
  ...
  async editStaffReply(
    ...
    if (inboxChannel && inboxChannel.isSendable()) {
