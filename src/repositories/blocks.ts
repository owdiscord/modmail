...
export async function block(
  ...
  await db`INSERT INTO blocked_users
...
export async function updateExpiryTime(
  ...
  await db`UPDATE blocked_users SET expires_at = DATE_ADD(NOW(), INTERVAL ${expires_at * 1000} MICROSECOND) WHERE user_id = ${user_id}`;
  return;
}
...
export async function getExpiredBlocks(
  ...
  const blockedUsers = await db<
    RowDataPacket & { user_id: string; duration: number }
  >`SELECT user_id, TIMESTAMPDIFF(MICROSECOND, blocked_at, expires_at) / 1000 duration FROM blocked_users WHERE expires_at IS NOT NULL AND expires_at <= ${now}`;
